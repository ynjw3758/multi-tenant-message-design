...
메시지 발송은 API 처리와 실제 외부 업체 호출을 분리하기 위해  
**Queue 기반 비동기 구조**로 설계하였다.

- API는 메시지를 PENDING 상태로 저장
- Queue에 이벤트를 발행
- Worker가 이벤트를 소비하여 실제 발송 수행
- 발송 결과에 따라 DB 상태 업데이트

---

### 📌 Queue 기술 선택 전략

현재 설계는 특정 메시지 브로커에 종속되지 않도록  
Queue를 **추상화된 개념**으로 설계하였다.

- 초기 단계: Redis 기반 Queue / 단순 비동기 처리 가능
- 트래픽 증가 시: Kafka, RabbitMQ 등 메시지 브로커로 확장 가능

이를 통해 스타트업 환경에서의 현실적 운영과  
향후 확장성을 동시에 고려하였다.
